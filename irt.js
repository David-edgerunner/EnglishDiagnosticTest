...
export const IRT = {
  ...
  thetaToScore(theta) {
    return Math.round(Math.max(0, Math.min(100, ((theta + 2.0) / 4.5) * 100)));
  },

  // Convert theta to CEFR level
  thetaToCEFR(theta) {
    const score = this.thetaToScore(theta);
    const t = IRT_CONFIG.cefrThresholds;
    if (score <= t.A2) return "A2";
    if (score <= t.B1) return "B1";
    if (score <= t.B2) return "B2";
  ...
  cefrToIELTS(cefr, score) {
    ...
    let rangeScores;
    if (cefr === "A2") rangeScores = [0, 20];
    else if (cefr === "B1") rangeScores = [21, 42];
    else if (cefr === "B2") rangeScores = [43, 68];
    else if (cefr === "C1") rangeScores = [69, 88];
    else rangeScores = [89, 100];
